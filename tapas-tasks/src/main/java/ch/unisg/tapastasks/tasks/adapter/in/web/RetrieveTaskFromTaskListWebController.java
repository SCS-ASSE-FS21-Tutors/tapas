...
/**
 * Controller that handles HTTP GET requests for retrieving tasks. This
 * controller implements the {@link RetrieveTaskFromTaskListUseCase} use case
 * using the {@link RetrieveTaskFromTaskListQuery} query.
 */
@RestController
public class RetrieveTaskFromTaskListWebController {
    ...
    /**
     * Retrieves a representation of task. Returns HTTP 200 OK if the request is
     * successful with a representation of the task using the Content-Type
     * "applicatoin/task+json".
     *
     * @param taskId the local identifier of the requested task (extracted from the
     *               task's URI)
