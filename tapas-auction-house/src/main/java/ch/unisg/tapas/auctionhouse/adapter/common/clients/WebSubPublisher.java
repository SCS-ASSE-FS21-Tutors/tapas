...
public class WebSubPublisher {
    ...
    public void notifyHub(String path) {
        ...
        if (webSubConfig.isProductionEnvironment()) {
            ...
            try {
                response = WebClient.post(webSubConfig.getHub(), parameters, CONTENT_TYPE_PRODUCTION);
            ...
            try {
                response = WebClient.post(webSubConfig.getHub() + "/publish", json, CONTENT_TYPE_DEVELOPMENT);
            } catch (IOException | InterruptedException ignored) { }
        }
